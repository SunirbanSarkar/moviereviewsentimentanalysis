# -*- coding: utf-8 -*-
"""
Created on Fri Jul 10 03:11:31 2020
...
class Review_sentiment:
    ...
    def __init__(self,path,epochs):
        ...
        with open(self.path_tokenizer,mode='rb') as handle:
            tokenizer = pickle.load(handle)

        self.EMBEDDING_FILE = path + 'glove.6B.100d.txt'
        def get_coefs(word, *arr): return word, np.asarray(arr, dtype='float32')
        embeddings_index = dict(get_coefs(*o.strip().split()) for o in open(self.EMBEDDING_FILE,encoding='utf-8'))  
    ...
    def _build_model(self):
        ...
        with tf.compat.v1.variable_scope('inputs'):
            self.X = tf.compat.v1.placeholder(shape=[None, self.sentence_length],dtype=tf.int32,name="X")
            print (self.X)
            self.y = tf.compat.v1.placeholder(shape=[None,1], dtype=tf.float32,name="y")
            self.emd_placeholder = tf.compat.v1.placeholder(tf.float32,shape=[self.n_words,self.embedding_dim]) 

        with tf.compat.v1.variable_scope('embedding'):
            # create embedding variable
            self.emb_W =tf.compat.v1.get_variable('word_embeddings',[self.n_words, self.embedding_dim],initializer=tf.compat.v1.random_uniform_initializer(-1, 1, 0),trainable=True,dtype=tf.float32)
            self.assign_ops = tf.compat.v1.assign(self.emb_W,self.emd_placeholder)
            
            # do embedding lookup
            self.embedding_input = tf.nn.embedding_lookup(params=self.emb_W,ids=self.X) 
        ...
        with tf.compat.v1.variable_scope('LSTM_cell'):
            self.cell = tf.compat.v1.nn.rnn_cell.BasicLSTMCell(self.hidden_states)

        
        # define the LSTM operation
        with tf.compat.v1.variable_scope('ops'):
            self.output, self.state = tf.compat.v1.nn.static_rnn(self.cell,self.embedding_input,dtype=tf.float32)
       
       
        with tf.compat.v1.variable_scope('classifier'):
            self.w = tf.compat.v1.get_variable(name="W", shape=[self.hidden_states,1],dtype=tf.float32)
            self.b = tf.compat.v1.get_variable(name="b", shape=[1], dtype=tf.float32)
        self.l2_loss = tf.nn.l2_loss(self.w,name="l2_loss")
        self.scores = tf.compat.v1.nn.xw_plus_b(self.output[-1],self.w,self.b,name="logits")
        ...
        self.loss = tf.reduce_mean(input_tensor=self.losses) + self.lambda1*self.l2_loss
        tf.compat.v1.summary.scalar('loss', self.loss)
        
        self.optimizer =  tf.compat.v1.train.AdamOptimizer(self.learning_rate).minimize(self.losses)
        

        self.correct_predictions = tf.equal(self.predictions,tf.round(self.y))
        print (self.correct_predictions)
 
        self.accuracy = tf.reduce_mean(input_tensor=tf.cast(self.correct_predictions, "float"), name="accuracy")
    ...
    def export_model(self,sess,saved_model_dir):
        ...
        inputs = {'Review': tf.compat.v1.saved_model.utils.build_tensor_info(in_text)}
        out_probability = sess.graph.get_tensor_by_name('positive_sentiment_probability:0')
        outputs = {'Positive Sentiment Polarity': tf.compat.v1.saved_model.utils.build_tensor_info(out_probability)}
        signature = tf.compat.v1.saved_model.signature_def_utils.build_signature_def(inputs=inputs,outputs=outputs,method_name=tf.saved_model.PREDICT_METHOD_NAME)
        ...
        open(self.path + "converted_model.tflite",mode="wb").write(tflite_model) 
    ...
    def _train(self):
        ...
        with tf.compat.v1.Session() as sess:
            ...
            tf.io.write_graph(sess.graph_def, self.path, 'model.pbtxt') 
